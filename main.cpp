...
{
    if (t0.y == t1.y && t0.y == t2.y) // degenerate triangle case
    {
        return;
    }

    // Sort the vertices and corresponding texture coordinates by ascending order of their y coordinates
    //  now we need to swap uvs as well
    if (t0.y > t1.y)
    {
        std::swap(t0, t1);
    }
...
int main(int argc, char **argv)
{
    ...
    { // draw the model
        // Move camera
        Matrix ModelView = lookat(eye, center, Vec3f(0, 10, 0));
